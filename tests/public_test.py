...
class BgpTest(TestCase):
    ...
    def setUp(self):
        ...
        with patch("city.UdpSocket") as city_udp:
            ...
            self.city = City(my_port=4, number_general=4)
        return super().setUp()
...
class BgpPublicTest(BgpTest):
    ...
    def test_listen_procedure_called_udpsocket_listen_once(self):
        self.mock_udp.listen.return_value = ("general_1~order=0", ("localhost", 123))
        self.loyal_general.listen_procedure()
        self.assertEqual(1, self.mock_udp.listen.call_count)

    def test_listen_procedure_return_list(self):
        self.mock_udp.listen.return_value = ("general_1~order=0", ("localhost", 123))
        result = self.loyal_general.listen_procedure()
        self.assertEqual(result, ["general_1", "order=0"])
    ...
    def test_concluding_action_equal(self):
        result = self.loyal_general.conclude_action([Order.ATTACK, Order.RETREAT])
        expected = f"general_{self.loyal_general.my_id}~action={Order.RETREAT}"
        self.assertEqual(result, expected)

    def test_concluding_action_return_message(self):
        result = self.loyal_general.conclude_action(
            [Order.ATTACK, Order.RETREAT, Order.ATTACK]
        )
    ...
    def test_start_method_called_all_procedure(
        self, mock_send, mock_listen, mock_conclude
    ):
    ...
    def test_supreme_general_send_message_to_other_generals(self):
        self.loyal_supreme_general.sending_procedure(
            "supreme_general", self.loyal_supreme_general.order
        )
    ...
    def test_supreme_general_called_all_procedures(self, mock_send, mock_conclude):
    ...
    def test_city_listen_according_number_general(self):
        ...
        self.city.start()
        self.assertEqual(self.city.number_general, self.mock_city_udp.listen.call_count)
...
class BgpPublicGrader(TestCase):
    ...
    def test_supreme_traitor_attack_return_attack(self, mock_random_order):
        mock_random_order.side_effect = [Order.ATTACK, Order.ATTACK, Order.RETREAT]
    ...
    def test_one_loyal_retreat_return_error(self, mock_random_order):
        mock_random_order.side_effect = [Order.ATTACK, Order.RETREAT, Order.RETREAT]
    ...
    @patch("node.General.get_random_order")
    @patch("node.SupremeGeneral.get_random_order")
    def test_two_traitors_attack_return_fail(
        self, mock_random_choice_sup, mock_random_choice_gen
    ):
        mock_random_choice_sup.side_effect = [Order.ATTACK, Order.ATTACK, Order.RETREAT]
